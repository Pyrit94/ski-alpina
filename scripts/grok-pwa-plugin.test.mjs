...
/**
 * A workspace with no site.json and no og card.
 *
 * `injectGrokPwaHead` consults the workspace on purpose: an omitted `site`
 * falls back to reading src/lib/og/site.json, and a public/og.jpg on disk
 * beats the og.grok.me placeholder. Both are right in production, and both
 * make an assertion about the *function* depend on whatever this app happens
 * to be branded as — once site.json said "Ski Builder", every test that left
 * `site`/`cwd` off started reading that instead of its own fixture. Tests that
 * are not about the workspace pin an empty one.
 */
const BARE_ROOT = mkdtempSync(join(tmpdir(), "grok-og-bare-"));
const bare = (ctx = {}) => ({ site: {}, cwd: BARE_ROOT, ...ctx });
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, bare({ appName: "Wild Race" }));
...
test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", bare({
    host: "wild-race.grok.me",
  }));
...
test("emits og:image for a public host and prefers a custom card", () => {
  const placeholder = injectGrokPwaHead("<html><head></head></html>", bare({
    appName: "Wild Race",
    host: "wild-race.grok.me",
    site: { title: "Wild Race" },
  }));
  ...
  const custom = injectGrokPwaHead("<html><head></head></html>", bare({
    appName: "Wild Race",
    host: "wild-race.grok.me",
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = injectGrokPwaHead("<html><head></head></html>", bare({
    host: "wild-race.grok.me",
  ...
  const invalid = injectGrokPwaHead("<html><head></head></html>", bare({
    host: "wild-race.grok.me",
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", bare({ appName: "Solo" }));
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector(bare({ appName: "Wild Race" }));
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", bare({ appName: "Wild Race" }));
